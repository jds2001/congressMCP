...
**Update 2026-08-24 — minimum-honesty fix landed; full fix approved and assigned.** Upstream #66 (`b0ce038`) shipped the deferred-fix fallback this entry specified: `search_bills` (and the sibling window tools) now fetch the API's 250-row maximum page, name the window in their result titles ("title/policy-area filter over the 250 most recently updated"), and return an honest miss message naming alternatives — the maintainer reports live verification ("climate" now finds bills the old window missed). **This is the honesty floor, not the fix:** D17's substring/OR matcher and D18's recency-window blindness both stand — HR 4631 remains unreachable by its own exact title on any recency window (asserted from the mechanism, not re-probed on #66's build; re-probe at acceptance). **Same day, the maintainer approved and assigned the full fix: GovInfo `/search` full-text over the BILLS collection replaces the window as the primary path.** Maintainer constraints, binding: #66's honest-window behavior is retained as the offline/GovInfo-down **fallback**, and a fallback response must remain distinguishable from a corpus zero; reuse the existing `X-Api-Key` + backoff GovInfo client; the fix composes with the PR-2 cache (a hit can warm straight into `search_bill_text`). **The spec for this work is `govinfo-search-spec.md` (this directory)** — the regression probes above are its preregistered acceptance floor. The joint masking constraint is satisfied by construction: the corpus search replaces the window and the matcher in one move.

**FIXED AND CLOSED 2026-08-25.** Implementation landed on `feature/govinfo-search` (`63b96e7..f1eee63`; spec `govinfo-search-spec.md` §6/§7); acceptance harness run by the maintainer, artifacts read by this session (`runs/govinfo-search/2026-08-25T015806Z/`): **both regression asserts recorded above now pass through the shipped tool** — `St. Louis RECA Readjustment Act` returns exactly HR 4631 (D18's unreachable-bill shape dead), `Radiation Exposure Compensation` returns 31 relevant hits (D17's zero dead), the drop-`Act` differential is structurally impossible under upstream implied-AND, and pagination pages the result set (four-page walk, zero duplicate ids, monotonic limit). Characterization tests landed per this register's standing rule (`test_govinfo_search_characterization.py`). The one non-passing acceptance cell (A7) is an instrument defect in the *fallback-disclosure* probe, tracked in the spec — it does not touch these entries' failure modes. **D17 and D18 CLOSED.**
...
- **Design proposals routed to the maintainer, recorded not ruled:** `search_bills` split out of the `bills` operation multiplexer (the 400-word docstring inside a generic bag is a real context cost); a `verbose` flag gating `timing`/`cache`/diagnostic fields — **resolved 2026-08-25: partially retracted by its author, adopted narrowly by the maintainer as fulltext amendment A7** (`timing` alone env-gated behind `CONGRESSMCP_VERBOSE`; `cache`/`version_resolution*`/disclosure notes stay always-on as diagnostic-on-failure; §4 otherwise stands); **`bills_amending(cite)` inverted index** — the strongest proposal here (the follow-up round also ruled Q11 local snippets and refined Q12 starved-result diagnostics in the spec; the reshuffle claim was retracted by its author), turning the corpus crawl the session performed by hand into one call over per-section citations already extracted. Its stated prerequisite is correct and matches the standing queue: **F36 first** — an inverted index over an extractor with a known left-side blind spot would be confidently, silently wrong.
...
**Deferred enhancement, recorded 2026-08-24 so it is not lost — server-wide trace coverage.** Extend `CONGRESSMCP_TRACE_DIR` from the bill-text tools (PR 1) and the rebuilt `search_bills` (`govinfo-search-spec.md`) to **every** operation, so all tool behavior is attributable from traces; once everything traces, the §17 harness's isolation surface and per-cell tool restriction become vestigial (attribution no longer requires restricting what a cell can reach). The prior blocker — not touching the pre-existing surface without upstream maintainer buy-in — is resolved: **buy-in obtained 2026-08-24** (the maintainer's read of the upstream exchange). Unscheduled; an enhancement, not a defect; scope it as its own PR when picked up, per this register's do-not-bundle rule.

**After PR 2, by maintainer priority (2026-08-20): the F27 error-shape convergence and D17/D18.** These outrank the lettered PRs below when PR 2 closes. D17 and D18 land together or window-first — the masking constraint in their joint entry. **2026-08-24: F27's convergence is reported landed upstream (#68; adjudication at the F27 entry, `fulltext/14-defect-priority.md`), and D17/D18's full fix is approved, assigned to the bill-text side, and specced at `govinfo-search-spec.md` — the active item.**
