...
- [ ] **Group F's verbatim-sourcing gap.** The six questions used were derived by someone who had read this spec, and were not run in the isolated cell — the manifest self-flags them as indicative only. Replace with verbatim questions from naive sources **and** run them in isolation before any Group F finding is recorded as a measurement. This is the one §17 gap the re-run did not close. *(Seeded 2026-08-22: the `119hr10115ih` session — see §18's consumer-session triage.)*

---

# 16b. Completion report — PR 2 (persistent cache)

> **Status 2026-08-22: complete.** Every §10 contract implemented, verified by V11 (14/14 scenarios) plus live certifications by this session throughout the build, and closed at the consumer layer by the two §17-PR2 runs. Same discipline as §16: enumerations, not narrative; *(reported)* marks figures not independently re-derived — of which this report has almost none, because nearly every claim was certified live by this session during the build.

## PR description — copy from the block below

---

### Persistent cache for the bill-text tools

**What this adds.** PR 1's bill-text tools re-fetched, re-parsed, and re-indexed the document on every call (~4–7 s on real bills). This PR adds the persistent cache specified alongside them: one SQLite file per package plus a derived manifest, so a **warm call answers in 30–60 ms end-to-end** (1.7–56 ms server compute) with **zero network** — and a 47-call deep analysis of an NDAA division now spends 2.6 s in the tools, total.

**Correctness posture, because a cache is a chance to be silently wrong.** The filesystem is authoritative and the manifest is derived — the server functions with the manifest deleted, stale, or corrupt (exercised, not asserted). Publication is atomic via an `os.link` claim — the verification sweep **caught a real race** (check-then-`os.replace` let two concurrent builders both "win") and the fix landed the same day. A found file is adopted only through a 7-point validation. Staleness is impossible by construction: any schema **or rendering** change discards and rebuilds, enforced by two CI tripwires — an AST pin over the rendering-determining source, and a **golden-build digest** of a real fixture build that moves if *anything* changes what a build produces.

**Response semantics.** `version_resolution` is now `fresh` / `cached` / `cached_offline` / `pinned` — and `cached_offline` carries a dated, active note ("resolved at …; the network was unavailable, so a newer version may exist"). Timing legs (`resolve_ms`/`download_ms`/`parse_ms`/`index_ms`) are each `null` when that leg did not run, so a warm hit is legible as one. Explicitly cached versions are fully queryable offline; `version=None` offline is best-effort and always labelled.

**Operations.** LRU eviction at a 500 MB cap (configurable) with best-effort cross-process leases; `congressmcp cache info` / `cache clear` (CLI by design — cache administration should not be in a model's reach, and a CLI works when a corrupt cache breaks the server); per-platform default paths; env tunables documented in the README, including the real cost of `CONGRESSMCP_CACHE_ENABLED=false` (full re-fetch and re-parse, every call).

**What's verified.** **V11: 14/14 scenarios, every check green** — LRU firing over NDAA-scale bills, every recovery row (manifest deleted / truncated / lying / absent rows / orphan files / newer-schema ignored in place), SIGKILL mid-build, two OS processes racing one package, a locked manifest, malformed XML, FTS5 absent, over-large documents. The run artifacts live under `runs/` (gitignored by design — large and reproducible); the sweep re-runs offline in minutes via `python -m tests.corpus.v11`, and the adjudicated record is in `documentation/fulltext/`. A second session (the spec session) independently reproduced the sweep and live-certified each build step against a real cache directory as it landed. At the consumer layer, two end-to-end runs: a deep-analysis prompt whose tool overhead fell from dominant to **1.4% of wall clock** with the answer's completeness-honesty intact, and a cross-version comparison task where the consumer produced **fully trace-grounded content diffs of both a 2-section bill and a 9.4 MB reconciliation act** — discovering the available versions, pinning them explicitly, and pairing searches across two different numbering schemes. The E2E harness itself gained a `cache` axis (cold = fresh directory per cell; warm = server-side pre-warm with a preflight that halts rather than mislabel).

**Known items, tracked and deliberately not in this PR:** rendering designators in emitted text and an `amends` recall gap on parenthetical Public-Law/USC-note citations (both found by a real research session *during* this PR's development, both measured — the latter at ~10% of amendatory units — and both queued next with their acceptance sets pinned); the server-wide error-shape convergence; the `search_bills` rebuild.
...
|---|---|---|
| §10 layout, manifest, tunables, CLI single-ownership (#21) | ✅ | `70ac942`/`a0799e5`; exit codes + import isolation + platform path certified live by spec session |
| Persistence path, adoption validation, loser-adopts | ✅ | `398f6d8`/`9fcf734`; warm path certified live (11 MB package, no sidecars) |
| `index_ms` null-on-hit; recovery; eviction; leases | ✅ | `58fe657`/`9eecead`/`0442a02`; recovery rows + two-act eviction certified live |
| Rendering tripwires (AST + golden-build) | ✅ | `eb19b59`/`3867e77`; scope ruled in §10 (golden = authoritative backstop) |
| Freshness TTLs, offline, `version_hit`, timing split (A6 closed) | ✅ | `070560c`; full ladder certified live incl. `cached_offline` note; A6's predicted signal observed |
| `pinned` resolution value | ✅ | `5242055`, ruled §10 |
| README (§12, seven bullets) | ✅ | `02861c5` |
| **V11** | ✅ **14/14** | `5e2bb22`; artifact `runs/v11/2026-08-22T035126Z/`; reproduced by spec session |
| **F34** publish race | found by V11, **FIXED + CLOSED** | `61821b4`; §10 amended to pin the atomic-claim mechanism |
| F31 keyless mislabel | **CLOSED live** | spec-session scrubbed-env call → `api_key_missing` |
| §17-PR2 Run A (A4 disposition) | **CLOSED as intended behavior** | `runs/2026-08-22T174941Z`; 2.6 s tool / 189.7 s wall, 47/47 hits, honesty held |
| §17-PR2 Run B (version-difference) | **prediction falsified, favorably** | `runs/2026-08-22T175330Z`; both arms trace-grounded; version discovery via provoked error observed |
| Harness cache axis | ✅ | `905bf82`/`325350a`; halt-don't-mislabel preflight |

## Carried forward (all named, none blocking)

**F35** (designators in emitted text — ruled, §5) + **F36** (parenthetical-PL/note `amends` recall — measured at ~10% of amendatory units, acceptance set pinned) land together next with one rendering-version bump. Then, by standing maintainer priority: **F27** §9-envelope convergence (cost update recorded — #53 deepened the legacy shape), **D17/D18** `search_bills` (joint fix constraint). Still open from PR 1: **F16** confirmation, **#17**, **Group F** verbatim rows (now seeded by the `hr10115` session). The §3 **version-discovery** requirement has its motivating case measured (VD-a discovered versions via a provoked error) and awaits its surface ruling at implementation time.
