...
| 4 | **§17 re-run** of affected cells (implementer-scripted CLI, §17) once F23/F24 are in | **UNBLOCKED** (F23/F24 landed) — not yet run. The measurement that tells whether the fixes reached the *consumer*; the highest-value remaining item. Haiku uses the shipped sibling heuristic (no amendment). |
