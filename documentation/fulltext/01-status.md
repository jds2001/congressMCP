...
| 3 | **F23 / F24** — §17 harness scores an un-exercised run clean; guard routers ship untested | **F23 FIXED `27be6e4`** (sys.executable + startup import probe + non-aborting `zero_trace_cells`; ratified — **one amendment routed back:** the Haiku capability cell needs an independent liveness proof, siblings can't tell abstention from a dead instrument there). **F24 open.** Land before trusting any further §17 re-run. |
