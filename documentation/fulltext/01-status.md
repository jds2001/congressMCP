...
| 5 | **F18 / F25 / F26** — quote-pair strip; `str(item)` version regex; whitespace-collapse desync | **F18 FIXED `449d38b`** (matched-pair-only; content-over-doubled-delimiter ruled in §6). **F25 FIXED `b464957`** (reads `formats[].url` by path, not the dict repr; composes with §3 step-2 fallback; F20 pattern reused). **F26 open — last of the row.** |
