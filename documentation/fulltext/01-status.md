...
| 3 | **F23 / F24** — §17 harness scores an un-exercised run clean; guard routers ship untested | **F23 FIXED `27be6e4`, fully ratified** (sys.executable + startup import probe + non-aborting `zero_trace_cells`; my Haiku-cell amendment was withdrawn — it measures disclosure-reading not adoption, and Haiku adopted last run). **F24 open.** Land before trusting any further §17 re-run. |
