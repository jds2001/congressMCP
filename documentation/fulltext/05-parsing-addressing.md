...
> **Designators render in display text — RULED 2026-08-22 (F35, §18; found by a real consumer session, verified live by the spec session).** The enum lived in the id, the TOC, and the child descriptors — and was **erased from the emitted text**: `119hr10115ih` §12's assembled text carries "Authorization of appropriations" with no `(e)`, so when §12(g) cross-references "section 12(e)" a consumer cannot check it from tool output, and the reader had to reconstruct the hierarchy the parser had discarded. **An amendatory bill is its enumeration** — internal cross-references, redesignation clauses ("paragraph (2)(F) (as redesignated by this Act)"), and quoted amendments all address text *by designator*. **Contract:** the display text of every **structural** unit below section level begins with its enum **exactly as the document writes it** (delimiters preserved: `(e)`, `(2)`, `(F)`), followed by its header where present; synthetic and chunk units are unchanged (they have no enum to render). This is a rendering change: it moves `byte_length`/chunk boundaries corpus-wide and **must bump the rendering version — the §10 tripwire and golden-build digest will force it, which is those guards working**. Acceptance: `119hr10115ih` §12 renders `(e)`/`(f)`/`(g)` before their headers and the §12(g)→12(e) cross-reference becomes checkable from the response alone.
>
> **CLOSED 2026-08-27; contract clarified at fix time.** The first implementation rendered designators only on child units and was falsified by this ruling's own acceptance — §12 is a single un-subdivided unit whose `(e)`/`(f)`/`(g)` live in flowing text. The contract binds the display text wherever the designator appears: rendering now happens in `extract_segments`, one path for flowing text and child units alike (adjudicated against `runs/f36/2026-08-27T010454Z/f35-s12-check.json`). SCHEMA_VERSION bumped 1→2. One correction to this ruling's confidence: the §10 tripwire did **not** force the bump — the golden inputs exercised neither changed path (blind spot closed at §10's tripwire ruling, `08-cache-storage.md`).
